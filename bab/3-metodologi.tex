...

{
	...
	\begin{algorithmic}[1]
		...
		\State $ModelResults \gets \{Model, ModelMAE, RMSE, R2Score, Accuracy\}$
		\State $Results \gets Results \cup ModelResults$
		\EndFor
		
		\State \textbf{Seleksi Model Terbaik}
		\State $BestOverallModel \gets \text{ArgMax}(Results, \text{key}=\text{Mean}(Accuracy))$ \Comment{Terbaik rata-rata}
		
		\State $BestPerTraitModels \gets \emptyset$
		\For{\textbf{each} $Trait \in \{\text{O, C, E, A, N}\}$}
		\State $BestModel \gets \text{ArgMax}(Results, \text{key}=Accuracy_{Trait})$ \Comment{Pemilihan model terbaik per \textit{trait}}
