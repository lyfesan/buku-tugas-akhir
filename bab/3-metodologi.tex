...
Bagian ini membahas mengenai metode-metode yang digunakan dalam melakukan interpretasi kepribadian \textit{Big Five} dari gambar yang diekstraksi pada video. Berdasarkan \autoref{fig:flowchart-penelitian}, alur penelitian meliputi praproses dataset video menjadi gambar, prediksi dan evaluasi nilai kepribadian, perancangan \textit{prompt} interpretasi kepribadian, interpretasi kepribadian dengan LLM dan evaluasi hasil interpretasi.
...
Dataset ChaLearn yang digunakan merupakan dataset dengan modalitas video. Oleh karena itu, diperlukan konversi data menjadi gambar statis. Setiap \textit{frame} yang diekstraksi dari video diperlakukan sebagai satu sampel data tunggal yang independen.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.75\linewidth]{gambar/flowchart-praproses.png}
	\caption{Diagram Alir Praproses Dataset}
	\label{fig:flowchart-praproses}
\end{figure}

Seperti yang ditunjukkan pada Gambar \ref{fig:flowchart-praproses}, tahapan praproses meliputi ekstraksi \textit{frame}, deteksi wajah utama dan \textit{cropping}, pengubahan ukuran gambar, augmentasi, dan normalisasi.
...
Setelah fitur visual diekstraksi oleh \textit{backbone} Transformer, model melakukan proses pelatihan model dengan melakukan prediksi nilai kepribadian yang dilanjutkan dengan \textit{backpropagation} untuk memperbarui bobot model. Alur mekanisme ini diilustrasikan pada Gambar \ref{fig:training-prediction}.
...
Sistem asesmen interpretasi kepribadian disusun sebagai sarana untuk mengakses model prediksi dan interpretasi kepribadian secara terintegrasi menggunakan \textit{Application Programming Interface} API agar dapat digunakan dengan mudah oleh pengguna.

\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{gambar/assesment-system.png}
	\caption{Diagram Alir Sistem Interpretasi Kepribadian}
	\label{fig:assesment-system}
\end{figure}
Sistem ini terdiri dari tiga komponen, yaitu API Inferensi, API Interpretasi, dan antarmuka pengguna. Mekanisme komunikasi antara ketiga komponen ditunjukkan oleh gambar \ref{fig:assesment-system}.

\subsubsection{API Inferensi}
API inferensi menyediakan akses bagi pengguna untuk melakukan prediksi nilai kepribadian \textit{Big Five}. Tugas dari API inferensi meliputi praproses gambar yang diterima dari pengguna dan inferensi menggunakan model prediksi yang menghasilkan nilai kepribadian \textit{Big Five} untuk digunakan pada tahap interpretasi selanjutnya. 
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{gambar/inference-interface.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=\linewidth]{gambar/interpretation-interface.png}
...
Evaluasi sistem dilakukan secara menyeluruh yang mencakup dua aspek utama, yaitu kinerja model regresi dalam memprediksi nilai kepribadian \textit{Big Five} dan kualitas teks interpretasi kepribadian yang dihasilkan oleh LLM.
...

{
	...
	\begin{xltabular}{\textwidth}{|
		...
		\hline
		\textbf{Model} & \textbf{Deskripsi} \arraybackslash \\ \hline
		\endhead
...
Selain itu, dilakukan uji ablasi dengan 4 jenis skenario yang dibuat untuk setiap model dengan tujuan mengetahui pengaruh skenario yang diujikan terhadap kinerja model. Detail mengenai skenario yang diujikan dapat dilihat pada \autoref{tab:skenario-uji}.  
...

{
	...
	\begin{xltabular}{\textwidth}{|c|
		...
		% --- Isi Data Tabel ---
		1 & Koherensi Gambar & Menilai kesesuaian dan konsistensi fitur fisik atau kesan visual dalam teks interepretasi kepribadian dengan gambar yang diberikan. \\ \hline
...

{
	...
	\begin{algorithmic}[1]
		...
		\State $BigFiveTraits \gets \emptyset$ \Comment{Standarisasi prediksi ke format Big Five}
...
Bagian ini membahas mengenai implementasi alur evaluasi sistem asesmen yang terdiri dari model prediksi regresi \textit{Big Five} dan interpretasi kepribadian.
